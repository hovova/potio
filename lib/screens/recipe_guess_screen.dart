...
class _RecipeGuessQuestion {
  final Drink correctDrink;
  final List<Drink> options;

  _RecipeGuessQuestion(this.correctDrink, this.options);
}

class RecipeGuessScreen extends StatefulWidget {
  const RecipeGuessScreen({super.key});

  @override
  State<RecipeGuessScreen> createState() => _RecipeGuessScreenState();
}

class _RecipeGuessScreenState extends State<RecipeGuessScreen> {
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                child: Column(
                  children: [
                    const Icon(Icons.receipt_long, color: potioCopper, size: 42),
                    const SizedBox(height: 16),
                    Text(
                      AppText.get(languageCode, 'which_drink_ingredients'),
                    ...
                    Wrap(
                      ...
                      children: currentQ.correctDrink.ingredients.map((ing) {
                        return Container(
                          padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
                          decoration: BoxDecoration(
                            color: potioDarkCoffee.withValues(alpha: 0.05),
                            borderRadius: BorderRadius.circular(12),
                            border: Border.all(color: potioCopperLight.withValues(alpha: 0.2)),
                          ),
                          child: Text(
                            ing,
                            style: const TextStyle(
